<template>
  <div
    ...
    :class="{
      selected: mainSelected && (props.layer === undefined || props.layer === selectedLayer),
      'is-trns': isTRNS,
...
const action = computed(() => {
  ...
  const layerIndex = typeof props.layer !== 'undefined' ? props.layer : selectedLayer.value
  if (!props.keymap[layerIndex]) return 'No layer'
  const keyCode = props.keymap[layerIndex][keyIndex]
...
watch(
  ...
  (_newValue) => {
    if (selectedKeys.value.has(props.keyIndex) && 
        (props.layer === undefined || props.layer === selectedLayer.value)) {
      mainSelected.value = true
      argsSelected.value = false
    } else {
      mainSelected.value = false

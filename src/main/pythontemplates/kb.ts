...
class POGKeyboard(KMKKeyboard):
    def __init__(self, features=['basic']):
        ...
        if "rgbkey" in features:
            from kmk.extensions.keyRGB import KeyRGB
            keyrgb = KeyRGB(
                pixel_pin=eval(pog.rgbKeyPin),
                num_pixels=pog.rgbKeyNumLeds,
                rgb_order=(1, 0, 2),
                val_limit=100,  # Maximum brightness level. Only change if you know what you are doing!
                hue_default=pog.rgbKeyOptions["hueDefault"],
                sat_default=pog.rgbKeyOptions["satDefault"],
                val_default=pog.rgbKeyOptions["valDefault"],
                animation_speed=pog.rgbKeyOptions["animationSpeed"],
                animation_mode=pog.rgbKeyOptions["animationMode"],
                breathe_center=pog.rgbKeyOptions["breatheCenter"],
                knight_effect_length=pog.rgbKeyOptions["knightEffectLength"],
            )
            #print("ASDASDASDASD")
            self.extensions.append(keyrgb)

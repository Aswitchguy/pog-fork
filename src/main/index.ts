import { app, shell, BrowserWindow, ipcMain, Menu, webContents } from 'electron'
...
const clampZoom = (level: number): number => {
  return Math.min(Math.max(level, MIN_ZOOM), MAX_ZOOM)
}

const logZoom = (action: string) => {
  const level = mainWindow?.webContents.getZoomLevel()
  console.log(`Zoom ${action} - Current level: ${level}`)
}
...
const createWindow = async () => {
  ...
  mainWindow.webContents.on('before-input-event', (event, input) => {
    ...
    if (input.control || input.meta) {
      switch (input.key) {
        case '=':
        case '+': {
          const newZoom = clampZoom(mainWindow.webContents.getZoomLevel() + 0.5)
  ...
  // Add mousewheel zoom
  mainWindow.webContents.on('mousewheel', (event, input) => {
...
ipcMain.on('zoom-in', () => {
  console.log('Main process: zoom-in received')
  ...
  if (contents) {
    ...
    contents.setZoomLevel(currentLevel + 0.5)
  }
})

ipcMain.on('zoom-out', () => {
  console.log('Main process: zoom-out received')
  const contents = webContents.getFocusedWebContents()
  if (contents) {
    const currentLevel = contents.getZoomLevel()
    console.log('Current zoom level:', currentLevel)
...
// Update wheel zoom
ipcMain.on('zoom-wheel', (_event, { deltaY }) => {
  if (!mainWindow) return
  const currentZoom = mainWindow.webContents.getZoomLevel()
  const newZoom = clampZoom(currentZoom + (deltaY < 0 ? 0.2 : -0.2))
  mainWindow.webContents.setZoomLevel(newZoom)
  logZoom(deltaY < 0 ? 'in (wheel)' : 'out (wheel)')
})
